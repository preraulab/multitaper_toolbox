...
multitaper_spectrogram_R <- function(data, fs, frequency_range=NULL, time_bandwidth=5, num_tapers=NULL, window_params=c(5,1),
  ...
  #         parallel (logical):  Use multiprocessing to compute multitaper spectrogram (default: FALSE)
  #         num_workers (numeric): Number of cpus to use if parallel is TRUE. (default: False). Note: if default is left
  #                                as False and parallel is True, the number of cpus used for multiprocessing will be
  #                                all available - 1.
  #         plot_on (logical): plot results (default: TRUE)
  #         verbose (logical): display spectrogram properties (default: TRUE)
  #         xyflip (logical): transpose the mt_spectrogram output (default: false)
  #
  # returns:
